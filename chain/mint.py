"""
Mint a finished piece as an ERC-721 on Base.

    python run.py mint 12            # dry run: builds the metadata, pins nothing, signs nothing
    python run.py mint 12 --live     # pins PNG + metadata to IPFS, mints, records tx and token id

The contract is contracts/ConnectomeCanvas.sol. `mint(to, uri, pngSha256, seed)`
puts the picture's hash and the sitting's seed on chain next to the token, so
the provenance the site shows is checkable against the block, not the site.
...
SITE_URL = os.environ.get("CANVAS_SITE_URL", "https://connectomecanvas.art")
...
def metadata_for(piece: dict, image_uri: str) -> dict:
    ...
    return {
        ...
        "image": image_uri,
        "external_url": f"{SITE_URL}/gallery.html#{piece['id']}",
        "attributes": [
...
def mint_piece(piece_id: int, dry_run: bool = True, network: str | None = None) -> dict:
    ...
    network = network or os.environ.get("CANVAS_NETWORK", "base")
    net = NETWORKS[network]
    contract_addr = os.environ.get("CANVAS_CONTRACT", "")
    png_path = GALLERY / piece["png"]

    if dry_run:
        meta = metadata_for(piece, "ipfs://<cid-of-png>")
        return {"dry_run": True, "network": network, "contract": contract_addr or "<CANVAS_CONTRACT unset>",
                "png": str(png_path), "metadata": meta}
    ...
    fn = contract.functions.mint(acct.address, f"ipfs://{meta_cid}", png_sha, int(piece["seed"]) & ((1 << 64) - 1))
